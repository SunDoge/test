...
function siteLanguage(){
	$("#select_language").find("a").click(function(){														 
		...
		//window.location.href="http://www.kqsgz.com/default.asp?siteLanguage="+languageId; //跳转本页面
		window.location.href="http://www.kqsgz.com/siteLanguage="+languageId; //跳转本页面
	})	
}
...
function messageOperate(userLabelId){
	$("#msgSubmit"+userLabelId).click(function(){ 	 
		...
		$.ajax({ 
			type: "POST", 
			url: "http://www.kqsgz.com/apply/messages/message_add.asp", 
			data:data, 
			cache:false, 
			error:function(){ 
...
function messageOperate1(userLabelId){
	...
	for(var i=1;i<=10;i++){		
		...
		$.ajax({ 
			type: "POST", 
			url: "http://www.kqsgz.com/apply/messages/onlineForm_add.asp", 
			data:data, 
			cache:false, 
			error:function(){ 
			createDialog({title:transKeyWords(globelVary.languageId,8),con:transKeyWords(globelVary.languageId,1),type:"error",width:300},[{name:transKeyWords(globelVary.languageId,5),action:function(){closeDialog("all");}}]); 
...
function discussOperate(userLabelId,distype,disId){
	var disContentObj=$("#saytext"+userLabelId+"dis"+disId);
		...
		$.ajax({ 
			type: "POST", 
			url: "http://www.kqsgz.com/apply/discuss/discuss_add.asp", 
			data:data, 
			cache:false, 
			error:function(){ 
			createDialog({title:transKeyWords(globelVary.languageId,8),con:transKeyWords(globelVary.languageId,1),type:"error",width:300},[{name:transKeyWords(globelVary.languageId,5),action:function(){closeDialog("all");}}]); 
...
function userLogin(userLabelId){
	$("#userSubmitLogin"+userLabelId).click(function(){
		...
		$.ajax({ 
			type: "POST", 
			url: "http://www.kqsgz.com/apply/member/userLogin.asp", 
			data:data, 
			cache:false, 
			error:function(){ 
				createDialog({title:transKeyWords(globelVary.languageId,8),con:transKeyWords(globelVary.languageId,1),type:"error",width:300},[{name:transKeyWords(globelVary.languageId,5),action:function(){closeDialog("all");}}]);
 ...
 $.extend({
        dLAdv:function(options){
            var defaults={
                ...
                leftSrc:"../../../../manager/images/dlAdvPic.jpg"/*tpa=http://www.kqsgz.com/manager/images/dlAdvPic.jpg*/,//左边图片路径
                leftAlt:"advertising",//左边广告图片的alt值
                rightSrc:"../../../../manager/images/dlAdvPic.jpg"/*tpa=http://www.kqsgz.com/manager/images/dlAdvPic.jpg*/,//右边图片路径
...
$.extend({
            pfAdv:function(options){
                var defaults={
                    ...
                    width:140,//图片大小
                    height:180,
                    imageSrc:"pfAdvPic.jpg"/*tpa=http://www.kqsgz.com/templates/sm_box/images/js/pfAdvPic.jpg*/,
                    step:1,
                    delay:30,
...
function openMen(){
	fwajaxStar();
	$.ajax({
		url:"http://www.kqsgz.com/apply/member/login.asp",
		data:{sType:""},
		type:"POST",
		dataType:"html",
		cache:"false",
	...
	$.ajax({
		...
		success:function(data){
			...
			createDialog({title:transKeyWords(globelVary.languageId,8),con:data.msg,type:"error",width:300},[{name:transKeyWords(globelVary.languageId,5),action:function(){closeDialog("self");}}])
				...
				//memberCenterShow('http://www.kqsgz.com/apply/member/memberInfo.asp');
				}					
			else 
...
function memberInfo_submit(className,type){	
			...
			var url;	
			if(type=="add"){
				url="http://www.kqsgz.com/apply/member/memberReg.asp";
				data.sType="add";
				}
			else if(type=="save"){
				url="http://www.kqsgz.com/apply/member/memberInfo.asp";
				data.sType="save";
				}
				fwajaxStar();
				$.ajax({
...
function forget_pw(){
	fwajaxStar();
	$.ajax({
		url:"http://www.kqsgz.com/apply/member/forgetPW.asp",
		data:{sType:""},
		type:"POST",
		dataType:"html",
		cache:"false",
...
function returnCart(){
	fwajaxStar();
	$.ajax({
		url:"http://www.kqsgz.com/apply/shopping/shoppingCar.asp",
		data:{sType:""},
		type:"POST",
		dataType:"html",
		cache:"false",
...
function payCart(){
	...
	fwajaxStar();
	$.ajax({
		url:"http://www.kqsgz.com/apply/shopping/shopFinish.asp",
		data:data,
		cache:false,
		type:"POST",
		error: function(){
...
function nowShopping(proId){
	...
	fwajaxStar();
	$.ajax({
		url:"http://www.kqsgz.com/apply/shopping/buyNow.asp",
		//dataType:"html",
		cache:false,
		type:"POST",
		data:data,
...
function addShoppingToCart(obj,proId){
	...
	//fwajaxStar();
	$.ajax({
		url:"http://www.kqsgz.com/apply/shopping/addCart.asp",
		dataType:"json",
		cache:false,
		type:"POST",
		data:data,
...
function getSessionTimeBool(){//每次ajax请求时调用,返回false后面的ajax不在调用,返回true表示未超时,可以继续执行后面的代码
	...
	//var bool=true;
	 if(globelVary.sessionTimeBool==1){
		...
		$.ajax({			
			url:"http://www.kqsgz.com/apply/member/logout.asp",
			type:"GET",
			cache:false,
			error: function(){
				createDialog({title:transKeyWords(globelVary.languageId,8),con:transKeyWords(globelVary.languageId,1),type:"error",width:300},[{name:transKeyWords(globelVary.languageId,5),action:function(){closeDialog("self");}}]);				
...
function getCode(){
	...
	fwajaxStar();
	$.ajax({
		url:"http://www.kqsgz.com/apply/member/mailVerification.asp",
...
function vote(userLabelId){
	...
	if (voteNameChecked === "")
	{
		...
		$.ajax({ 
			type: "POST", 
			url: "http://www.kqsgz.com/apply/vote/vote_add.asp", 
...
function voteResult(userLabelId){
	$.ajax({ 
		type: "POST", 
		url: "http://www.kqsgz.com/apply/vote/vote_result.asp", 
